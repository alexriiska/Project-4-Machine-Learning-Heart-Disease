...
## Conclusion:
Supervised learning is a machine learning model where a variable or variables is to represent a response. The goal of supervised learning is to make inferences or make predictions. Algorithms are used to fit and select models for classification or prediction. In our case, we are using classification.
...
In future approaches, we would like to try hyper-tuning and set different hyperparameters to see if the result differs.
We’re also interested in seeing a larger set of female patients since our current data is shown as dominantly male.
In addition, 21% of the dataset includes female readings and their diagnosis, and ~19% of all cholesterol readings were 0. We suspected that these zero values were originally null and changed to zero during the initial data collecting step. In our next attempt, we plan to remove all the outliers in the Cholesterol variable, (although that might decrease the size of the overall data) as we suspect it might improve our model's accuracy scores.
