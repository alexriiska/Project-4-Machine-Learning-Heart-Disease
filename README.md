...
In addition, 21% of the dataset includes female readings and their diagnosis, and ~19% of all cholesterol readings were 0. We suspected that these zero values were originally null and changed to zero during the initial data collecting step. In our next attempt, we plan to remove all the outliers in the Cholesterol variable, (although that might decrease the size of the overall data) as we suspect it might improve our models' accuracy scores.
